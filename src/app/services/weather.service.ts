...
export class WeatherService {
	...
	public getEuropeanCities(cnt = 5, lat = 51.5085, lon = -0.1257): Observable<
		{
			name: string;
			temp: string;
			sunrise: number;
	...
	> {
		return this.http
			.get(
				`${environment.apiUrl}/find?lat=${lat}&lon=${lon}&cnt=${cnt}&appid=${environment.apiKey}`
			)
			.pipe(
				...
				map((comnbinedInfo) => comnbinedInfo as {
					name: string;
					temp: string;
					sunrise: number;
					sunset: number;
	...
	public getWeatherReport(city: string): Observable<{
		...
		return this.http
			.get(`${environment.apiUrl}/weather?q=${city}&appid=${environment.apiKey}`).pipe(
				map((response: any) => {
					return {
						name: response.name,
	...
	public noOfDaysReport(name: string, cnt = 40): Observable<{
		main: string,
		noOfDays: {
			date: number,
			dateString: string,
		...
		return this.http
			.get(
				`${environment.apiUrl}/forecast?q=${name}&appid=${environment.apiKey}&cnt=${cnt}`
			)
			.pipe(
				map((response: any) => ({
					main: response.city,
